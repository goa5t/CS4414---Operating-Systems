...
int fs_create(char * name){
    ...
    //printf("in fs_create, tmp_name = %s\n", tmp_name);
    for(i=0; i < 8; i++){
    ...
    for(i=0; i < 8; i++){
        if(directory[i].status == 0){
            ...
            directory[i].fn = tmp_name;
            created = 1;
            return 0;
        }
    }
    if(i==8 && created == 0){
        //8 files already created
        return -1;
    }
    if(created==0){
...
int fs_open(char * name){
    //printf("calling open for name=%s\n", name);
    // printMeta();
    int i=0;
    int p=0;
    int found = 0;
    ...
    for(i=0; i < 4; i++){
        if(OFT[i].status == 1){
            ...
            //printf("testing seg\n");
            if(strcmp(tmp_name, directory[OFT[i].dir_index].fn) == 0){
    ...
    for(p=0; p < 8 && !found; p++){
        //printf("comparing->%s<- with->%s<-\n", tmp_name, directory[p].fn);
        if(strcmp(tmp_name, directory[p].fn)==0){
    ...
    //update OFT if file exists in directory
    for(i=0; i < 4; i++){
        if(OFT[i].status ==0 && found==1){
            OFT[i].status = 1;
            OFT[i].file_offset = 0;
            OFT[i].dir_index = dir;
            fildes = i;
            i = 4;
            //printf("File = %s is assigned fildes = %i, index at directory[%i]\n", tmp_name, fildes, dir);
        }
    }
...
int fs_delete(char * name){
    ...
    int dir = -1;
    for(i=0; i < 8; i++){
        if(directory[i].status == 1 && strcmp(tmp_name, directory[i].fn) == 0){
            //found the file
            dir = i;
        }
    }
...
int fs_write(int fildes, void *buf, size_t nbyte){
	...
	int old_end = 0;
	if(offset % 16 == 0)
		offset_block = offset / 16;
	else
		offset_block = (offset / 16) + 1;
    if((offset + nbyte) % 16 ==0)
        total_blocks = ((new_length) / 16);
    else
        total_blocks = ((new_length) / 16) + 1;
	...
	int old_length = directory[dindex].length;
	if((old_length % 16) == 0)
		allocatedBlocks = old_length / 16;
	else
		allocatedBlocks = (old_length / 16) + 1;
	//printf("allocatedBlocks = %i\n", allocatedBlocks);
	//printf("total blocks = %i\n", total_blocks);
	int newBlocks = total_blocks - allocatedBlocks;
	int block = directory[dindex].block;
	int b=0;
	while(b < allocatedBlocks-1){
		if(tmp_offset < 16)
			old_end = block;
		block = FAT[block-32].next_block;
	...
	//block is last allocated block for the file
	while(newBlocks > 0){
	// allocate a new block for each in teh FAT
		int p=0;
		for(p=0; p < 32; p++){
			if(FAT[p].status==0){
				//printf("block = %i\n", block);
				//printf("setting FAT[%i]status to 1, FAT[%i].nextblock=%i\n", p, block-32, p+32);
				FAT[p].status = 1;
				FAT[block-32].next_block = p+32;
				block = p;
				p = 32;
			}
		}
	...
	while(offset > 16)
		offset -=16;
	char * readIn = calloc(16, 1);
	block_read(old_end, readIn);
	//readIn now contains the block where the offset is located
	//printf("readIn->%s<-\n", readIn);
	char * toWrite = calloc(16,1);
	int i=0;
	for(i=0; i < offset; i++){
		toWrite[i] = readIn[i];
	}
	int y=0;
	int newIndex = 0;
	for(y=i; y < 16 && newIndex < nbyte; y++){
		toWrite[y] = buf_char[newIndex];
		newIndex++;
	}
	block_write(old_end, toWrite);
	block = FAT[old_end - 32].next_block;
	//printf("Just wrote to block= %i, string->%s<-\n", old_end, toWrite);
	int z=0;
	//printf("totalblocks = %i, offsetblocks = %i\n", total_blocks, offset_block);
	for(z=0; z < (total_blocks - offset_block); z++){	
		char * toWrite2 = calloc(16,1);
		i=0;
		while(newIndex < nbyte && i < 16){
			toWrite2[i] = buf_char[newIndex];
			i++;
			newIndex++;
		}
		block_write(block, toWrite2);
		//printf("Just wrote to block= %i, string->%s<-\n", block, toWrite2);
		int nb = FAT[block - 32].next_block;
		block = nb;
	}
	}
...
int fs_truncate (int fildes, off_t length){
    ...
    fs_lseek(fildes, (OFT[fildes].file_offset * -1));
    fs_read(fildes, newString, length);
    //printf("in truc, newString->%s<-\n", newString);
    fs_write(fildes, newString, length);
...
void updateDir(){
    ...
    for(i=dir_start; i < dir_end + 1; i++){
        ...
        //file length, 3 bytes
        buf[7] = (directory[dir_index].length / 100)+ '0';
        buf[8] = (directory[dir_index].length / 10)+ '0';
        buf[9] = (directory[dir_index].length % 10)+ '0';
